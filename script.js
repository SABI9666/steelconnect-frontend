...
function initializeApp() {
    ...
    // --- SETTINGS / PROFILE COMPLETION TOGGLE LOGIC ---
    // This logic runs once the page loads and will work whenever the settings page is rendered.
    // It finds the necessary elements for toggling between the settings view and the profile update view.

    // Use event delegation on a parent container to ensure listeners work even if content is re-rendered.
    const mainContainer = document.querySelector('.main-container');
    if (mainContainer) {
        mainContainer.addEventListener('click', function(event) {
            // Logic for showing the profile form
            if (event.target.matches('#edit-profile-btn') || event.target.closest('#edit-profile-btn')) {
                const settingsContainer = document.querySelector('.settings-container');
                const profileCompletionContainer = document.querySelector('.profile-completion-container');

                if (settingsContainer && profileCompletionContainer) {
                    settingsContainer.style.display = 'none';
                    profileCompletionContainer.style.display = 'block';
                }
            }

            // Logic for going back to the settings page
            if (event.target.matches('#back-to-settings-btn') || event.target.closest('#back-to-settings-btn')) {
                event.preventDefault();
                const settingsContainer = document.querySelector('.settings-container');
                const profileCompletionContainer = document.querySelector('.profile-completion-container');

                if (settingsContainer && profileCompletionContainer) {
                    profileCompletionContainer.style.display = 'none';
                    settingsContainer.style.display = 'grid'; // Use grid to match original style
                }
            }
        });
    }
    ...
    return `
        ...
        <div class="settings-container">
            <div class="settings-card">
                <h3><i class="fa fa-user-check" style="margin-right: 8px; color: var(--primary-color);"></i> Profile Status & Completion</h3>
                <p style="margin: 8px 0 16px;">
                    Keep your professional information up-to-date to ensure your profile is approved and visible to clients.
                </p>
                <button id="edit-profile-btn" class="btn btn-primary">
                    <i class="fa fa-edit"></i> Update My Profile
                </button>
            </div>
