...
async function renderConversationView(conversationOrId) {
        ...
        <div class="chat-container modern-chat">
            <div class="chat-header">
                ...
                <div class="chat-header-info">
                    <div class="chat-avatar" style="background-color: ${avatarColor}">${otherParticipant ? otherParticipant.name.charAt(0).toUpperCase() : '?'}</div>
                    <div class="chat-details">
                        <h3>${otherParticipant ? otherParticipant.name : 'Conversation'}</h3>
                        <p class="chat-project"><i class="fas fa-briefcase"></i>${conversation.jobTitle || ''}</p>
                    </div>
                </div>
            </div>
            <div class="chat-messages" id="chat-messages-container"><div class="loading-messages">...</div></div>
            <div class="chat-input-area">
                <form id="send-message-form" class="message-form">
                    <div class="message-input-container">
                        ...
                        <button type="submit" class="send-button" title="Send message"><i class="fas fa-paper-plane"></i></button>
                    </div>
                </form>
            </div>
        </div>
    ...
    try {
        ...
        if (messages.length === 0) {
            messagesContainer.innerHTML = `<div class="empty-messages">...</div>`;
        } else {
            messagesContainer.innerHTML = messages.map((msg, index) => ``).join('');
        }
        messagesContainer.scrollTop = messagesContainer.scrollHeight;
    } catch (error) {
        messagesContainer.innerHTML = `<div class="error-messages">...</div>`;
    }
}
...
// --- NEWLY ADDED/VERIFIED ESTIMATION TOOL FUNCTIONS ---
            ...
            <form id="post-job-form" class="modern-form post-job-form">
                <div class="form-section">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group"><label class="form-label"><i class="fas fa-calendar-alt"></i> Project Deadline</label><input type="date" class="form-input" name="deadline" required></div>
                    </div>
                    <div class="form-group"><label class="form-label"><i class="fas fa-tools"></i> Required Skills</label><input type="text" class="form-input" name="skills" placeholder="e.g., AutoCAD, Revit, Structural Analysis, Steel Design"><small class="form-help">Separate skills with commas</small></div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-group"><label class="form-label"><i class="fas fa-paperclip"></i> Project Attachments</label><input type="file" class="form-input file-input" name="attachment" accept=".pdf,.doc,.docx,.dwg,.jpg,.jpeg,.png"><small class="form-help">Upload drawings or specifications (Max 10MB)</small></div>
                </div>
