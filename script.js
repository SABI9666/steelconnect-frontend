...
// Google OAuth Client ID (public identifier, safe for frontend)
...
async function handleRegister(event) {
    ...
    // Attach referral code: from form field first, then sessionStorage fallback
    const formRefCode = form.regReferralCode ? form.regReferralCode.value.trim().toUpperCase() : '';
    const savedRefCode = formRefCode || sessionStorage.getItem('steelconnect_referral_code') || '';
    if (savedRefCode) {
...
// Track the current Google sign-in context (login/register)
let _googleSignInContext = 'login';
...
function renderGoogleButtons() {
    ...
    const targets = [
        { id: 'header-google-btn', type: 'icon', theme: 'outline', size: 'medium', shape: 'pill' },
        { id: 'hero-google-login-btn', type: 'standard', theme: 'outline', size: 'large', text: 'continue_with' },
        { id: 'gateway-google-render', type: 'standard', theme: 'outline', size: 'large', text: 'signin_with' }
    ];
    targets.forEach(t => {
...
async function fetchTurnCredentials() {
    try {
        const backendUrl = IS_LOCAL ? 'http://localhost:10000' : 'https://steelconnect-backend.onrender.com';
        // Use AbortController for proper fetch timeout (fetch API does not support timeout option)
        const controller = new AbortController();
        ...
        if (err.name === 'AbortError') {
            console.warn('[VOICE] TURN credential fetch timed out after 8s, using built-in defaults');
        } else {
...
function getDefaultIceServers() {
    return [
        // --- Multi-provider STUN servers for redundancy ---
        // Google (primary - widely accessible)
        { urls: 'stun:stun.l.google.com:19302' },
        { urls: 'stun:stun1.l.google.com:19302' },
        { urls: 'stun:stun2.l.google.com:19302' },
        { urls: 'stun:stun3.l.google.com:19302' },
        { urls: 'stun:stun4.l.google.com:19302' },
        // Twilio (US/EU/Asia presence - works well from India & UK)
        { urls: 'stun:global.stun.twilio.com:3478' },
        // Cloudflare (edge nodes in India & UK)
        ...
        // --- OpenRelay TURN (free public fallback) ---
        // Provides additional relay path if Metered.ca is down/expired
        {
...
// Uses adaptive sample rate for better compatibility on low-bandwidth international links
...
async function createPeerConnection(forceRelay) {
    ...
    const config = {
        ...
        // Increased pool size for better international NAT traversal
    ...
    voiceCallState._hasRelayCandidates = false;
    voiceCallState._iceGatheringTimer = null;
    ...
    voiceCallState.peerConnection.onicecandidate = (event) => {
        if (event.candidate && voiceCallState.socket) {
            ...
            if (candidateType === 'relay') voiceCallState._hasRelayCandidates = true;
            ...
            if (voiceCallState._iceGatheringTimer) {
                clearTimeout(voiceCallState._iceGatheringTimer);
                voiceCallState._iceGatheringTimer = null;
            }
            console.log(`[VOICE] ICE gathering complete. Total candidates: ${voiceCallState._iceCandidateCount} | relay: ${voiceCallState._hasRelayCandidates}`);
            // If zero relay candidates were gathered, TURN servers are unreachable
            ...
            } else if (!voiceCallState._hasRelayCandidates && !voiceCallState._forceRelay) {
                console.warn('[VOICE] WARNING: No relay candidates - TURN credentials may be expired. Cross-region calls may fail.');
            }
        }
    };

    // --- ICE gathering timeout (prevents stalling on restrictive networks) ---
    voiceCallState._iceGatheringTimer = setTimeout(() => {
        if (!voiceCallState._iceGatheringComplete && voiceCallState.peerConnection) {
            console.warn(`[VOICE] ICE gathering timeout (15s) - gathered ${voiceCallState._iceCandidateCount} candidates so far`);
            voiceCallState._iceGatheringComplete = true;
            // If we have no relay candidates after timeout, immediately try relay mode
            if (!voiceCallState._hasRelayCandidates && !voiceCallState._forceRelay && voiceCallState._iceCandidateCount > 0) {
                console.warn('[VOICE] No relay candidates after timeout, scheduling relay retry');
            }
        }
    }, 15000);
    ...
    voiceCallState.peerConnection.oniceconnectionstatechange = () => {
        ...
        if (state === 'failed') {
            if (!voiceCallState._forceRelay && voiceCallState._iceRestartAttempts === 0) {
                ...
                retryWithRelay();
            } else if (voiceCallState._iceRestartAttempts < 4) {
                voiceCallState._iceRestartAttempts++;
                console.log(`[VOICE] ICE restart attempt ${voiceCallState._iceRestartAttempts}/4`);
                attemptIceRestart();
            }
        } else if (state === 'checking') {
            // Set a timeout for ICE checking phase - if stuck for too long, force relay
            if (!voiceCallState._iceCheckingTimer) {
                voiceCallState._iceCheckingTimer = setTimeout(() => {
                    if (voiceCallState.peerConnection &&
                        voiceCallState.peerConnection.iceConnectionState === 'checking' &&
                        !voiceCallState._forceRelay) {
                        console.warn('[VOICE] ICE checking stuck for 12s - switching to relay mode');
                        retryWithRelay();
                    }
                }, 12000);
            }
            ...
            if (voiceCallState._iceCheckingTimer) {
                clearTimeout(voiceCallState._iceCheckingTimer);
                voiceCallState._iceCheckingTimer = null;
            }
        }
    };
    ...
    voiceCallState.peerConnection.onconnectionstatechange = () => {
        ...
        if (state === 'failed') {
            ...
            // Give cross-region connections (India <-> UK) time to recover
            // International routes can have 5-10s packet loss spikes
            voiceCallState._disconnectTimer = setTimeout(() => {
                if (voiceCallState.peerConnection &&
                    voiceCallState.peerConnection.connectionState === 'disconnected') {
                    // Before giving up, try one ICE restart
                    if (voiceCallState._iceRestartAttempts < 4) {
                        console.log('[VOICE] Connection disconnected, attempting ICE restart recovery...');
                        voiceCallState._iceRestartAttempts++;
                        attemptIceRestart();
                    } else {
...
function startAudioFlowCheck() {
    ...
    if (voiceCallState._audioCheckTimer2) clearTimeout(voiceCallState._audioCheckTimer2);

    // First check after 4 seconds
    voiceCallState._audioCheckTimer = setTimeout(() => doAudioFlowCheck(false), 4000);
    // Second check after 8 seconds (confirms no audio if first check passed but was borderline)
    voiceCallState._audioCheckTimer2 = setTimeout(() => doAudioFlowCheck(true), 8000);
}

async function doAudioFlowCheck(isFinalCheck) {
    ...
    try {
        ...
        console.log(`[VOICE] Audio flow check (final=${isFinalCheck}): bytesReceived=${bytesReceived}, packetsReceived=${packetsReceived}, packetsLost=${packetsLost}, jitter=${jitter}`);
        ...
        if (!audioReceived && !voiceCallState._forceRelay) {
            ...
            retryWithRelay();
        } else if (!audioReceived && voiceCallState._forceRelay && isFinalCheck) {
            ...
            if (voiceCallState._iceRestartAttempts < 4) {
                ...
                console.error('[VOICE] All audio recovery attempts exhausted');
                showNotification('Audio connection issue. Try ending and restarting the call.', 'warning');
            }
        } else if (audioReceived) {
            console.log('[VOICE] Audio is flowing normally');
            // Log connection quality for diagnostics
            if (jitter > 0.05) {
                console.warn(`[VOICE] High jitter detected: ${(jitter * 1000).toFixed(1)}ms - possible network congestion on international route`);
            }
        }
    } catch (e) {
...
async function attemptIceRestart() {
    try {
        ...
        console.log(`[VOICE] ICE restart attempt ${voiceCallState._iceRestartAttempts}/4 - refreshing ICE candidates...`);
        ...
        // If ICE restart fails and we haven't tried relay, switch to relay
        if (!voiceCallState._forceRelay) {
            console.log('[VOICE] ICE restart failed, falling back to relay mode...');
            retryWithRelay();
        }
    }
}
...
function endCallCleanup() {
    ...
    // Clear audio flow check timers
    if (voiceCallState._audioCheckTimer) {
        clearTimeout(voiceCallState._audioCheckTimer);
        voiceCallState._audioCheckTimer = null;
    }
    if (voiceCallState._audioCheckTimer2) {
        clearTimeout(voiceCallState._audioCheckTimer2);
        voiceCallState._audioCheckTimer2 = null;
    }

    // Clear ICE gathering and checking timers
    if (voiceCallState._iceGatheringTimer) {
        clearTimeout(voiceCallState._iceGatheringTimer);
        voiceCallState._iceGatheringTimer = null;
    }
...
function showAuthGateway() {
    ...
    // Google button in gateway uses renderButton() directly — no clone needed
    // Re-render Google button in case gateway was re-opened
    if (googleBtn) {
        const renderTarget = document.getElementById('gateway-google-render');
        if (renderTarget && ensureGoogleInitialized()) {
            google.accounts.id.renderButton(renderTarget, {
                type: 'standard', theme: 'outline', size: 'large',
                text: 'signin_with', width: renderTarget.offsetWidth || 250,
                click_listener: () => { _googleSignInContext = 'login'; }
            });
        }
    }
...
function renderAppSection(sectionId) {
    ...
    else if (sectionId === 'referral-rewards') {
        renderReferralRewardsPage();
    }
...
function renderAIEstimateResult(estimate, projectInfo) {
            ...
            <div style="display:grid;gap:8px;">
                ${confidenceFactors.map(f => {
                    ...
                    return `<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
                        <span style="min-width:90px;max-width:140px;flex-shrink:0;font-size:0.82rem;color:#475569;">${f.name}</span>
                        ...
                        <span style="min-width:32px;text-align:right;font-size:0.82rem;font-weight:700;color:${fColor};">${f.score}%</span>
                        <span style="font-size:0.68rem;color:#94a3b8;">w:${f.weight}</span>
                    </div>`;
                }).join('')}
            </div>
        </div>` : '';
    ...
    const multiPassBarHTML = (passesCompleted > 0 || multiPassMeta.engineVersion) ? `
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin:8px 0 16px;font-size:0.75rem;">
    ...
    if (hasMaterialSchedule) {
        ...
        if (mepItems.length > 0) {
            ...
            mepTableHTML = `<div style="margin-bottom:16px;">
                <h4 style="margin-bottom:8px;color:#7c3aed;display:flex;flex-wrap:wrap;align-items:center;gap:4px;justify-content:space-between;"><span><i class="fas fa-plug"></i> MEP (Mechanical / Electrical / Plumbing)</span> <span style="font-size:0.8rem;font-weight:600;color:#7c3aed;">${curr}${fmtNum(mepCost)}</span></h4>
        ...
        if (archItems.length > 0) {
            ...
            archTableHTML = `<div style="margin-bottom:16px;">
                <h4 style="margin-bottom:8px;color:#b45309;display:flex;flex-wrap:wrap;align-items:center;gap:4px;justify-content:space-between;"><span><i class="fas fa-paint-roller"></i> Architectural Finishes</span> <span style="font-size:0.8rem;font-weight:600;color:#b45309;">${curr}${fmtNum(archCost)}</span></h4>
        ...
        if (manpower.totalLaborHours > 0 || crewBreakdown.length > 0) {
            manpowerHTML = `<div style="margin-top:20px;padding:16px;background:#faf5ff;border:1px solid #e9d5ff;border-radius:10px;overflow-x:hidden;">
        ...
        if (markups.grandTotalWithMarkups > 0 || markups.subtotalDirectCost > 0) {
            ...
            markupsHTML = `<div style="margin-top:20px;padding:12px;background:#fefce8;border:1px solid #fde68a;border-radius:10px;overflow-x:hidden;">
                <h4 style="margin-bottom:12px;color:#92400e;font-size:0.9rem;"><i class="fas fa-percentage"></i> BOQ Markups & Final Total</h4>
                <div style="display:flex;flex-direction:column;gap:6px;">
                    <div style="display:flex;justify-content:space-between;padding:8px 10px;background:#fff;border-radius:6px;font-weight:600;font-size:0.88rem;gap:8px;flex-wrap:wrap;">
                    ...
                    <div style="display:flex;justify-content:space-between;padding:10px 12px;background:#92400e;color:#fff;border-radius:6px;font-weight:700;font-size:0.95rem;gap:8px;flex-wrap:wrap;">
...
function buildSidebarNav() {
    ...
    links += `
        ...
        <hr class="sidebar-divider">
        <div class="sidebar-section-title">Rewards</div>
        <a href="#" class="sidebar-nav-link" data-section="referral-rewards">
            <i class="fas fa-gift fa-fw"></i>
            <span>Referral Rewards</span>
            <span class="nav-badge" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;">FREE</span>
        </a>
        <hr class="sidebar-divider">
...
function renderEstimationBlockedPlans(plans) {
    ...
    // Plan display config matching the app subscription page (exclude free plan)
    ...
    if (plans) {
        plansHtml = estPlanConfigs.map(cfg => {
            ...
            const btnLabel = plan.isPayPerUse ? 'Buy Estimate' : 'Subscribe';
...
// Show sign-in popup with Google + Email options when clicking Subscribe on a plan
function showEstimationSignInPopup(planId) {
    // Store selected plan
    if (planId) {
    ...
    const existing = document.getElementById('estimation-signin-popup');
    if (existing) existing.remove();

    const popup = document.createElement('div');
    popup.id = 'estimation-signin-popup';
    popup.innerHTML = `
        <div onclick="closeEstimationSignInPopup()" style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:10000;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease;">
            <div onclick="event.stopPropagation()" style="background:#fff;border-radius:16px;padding:28px 24px;max-width:380px;width:90%;box-shadow:0 20px 60px rgba(0,0,0,0.2);position:relative;">
                <button onclick="closeEstimationSignInPopup()" style="position:absolute;top:12px;right:12px;background:none;border:none;font-size:18px;color:#94a3b8;cursor:pointer;padding:4px;line-height:1;" title="Close">
                    <i class="fas fa-times"></i>
                </button>
                <div style="text-align:center;margin-bottom:20px;">
                    <div style="width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,#dbeafe,#c7d2fe);display:flex;align-items:center;justify-content:center;margin:0 auto 10px;">
                        <i class="fas fa-user-shield" style="font-size:18px;color:#4f46e5;"></i>
                    </div>
                    <h3 style="font-size:17px;font-weight:700;color:#1e293b;margin:0 0 4px;">Sign in to Subscribe</h3>
                    <p style="font-size:12px;color:#64748b;margin:0;">Choose your preferred sign-in method</p>
                </div>

                <!-- Google Sign-In -->
                <div id="estimation-popup-google" style="display:flex;justify-content:center;margin-bottom:12px;min-height:44px;"></div>
                ...
                <!-- Email Sign In -->
                <button onclick="closeEstimationSignInPopup();selectEstimationPlan(null, 'login')" style="width:100%;padding:11px;background:#fff;color:#374151;border:2px solid #e5e7eb;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:8px;" onmouseover="this.style.borderColor='#6366f1';this.style.color='#6366f1'" onmouseout="this.style.borderColor='#e5e7eb';this.style.color='#374151'">
                    <i class="fas fa-envelope"></i> Sign In with Email
                </button>

                <!-- Create Account -->
                <button onclick="closeEstimationSignInPopup();selectEstimationPlan(null, 'register')" style="width:100%;padding:11px;background:#fff;color:#374151;border:2px solid #e5e7eb;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:8px;" onmouseover="this.style.borderColor='#22c55e';this.style.color='#16a34a'" onmouseout="this.style.borderColor='#e5e7eb';this.style.color='#374151'">
    ...
    // Render Google Sign-In button in popup
    setTimeout(() => {
        const googleTarget = document.getElementById('estimation-popup-google');
...
function closeEstimationSignInPopup() {
    const popup = document.getElementById('estimation-signin-popup');
...
// ================================================================
// REFERRAL REWARDS - Professional Sharing & Rewards System
// ================================================================
async function renderReferralRewardsPage() {
    const container = document.getElementById('app-container');
    if (!container) return;

    const userType = appState.currentUser?.type || 'contractor';
    const isContractor = userType === 'contractor';
    const rewardLabel = isContractor ? 'Free Estimation / Analysis' : 'Free Quote';
    const rewardIcon = isContractor ? 'fa-chart-line' : 'fa-file-invoice';
    ...
    <div class="referral-rewards-page">
        ...
        <div class="referral-how-it-works">
            <h2 class="referral-section-title"><i class="fas fa-info-circle"></i> How It Works</h2>
        ...
        <div class="referral-stats-section" id="referral-stats-container">
            <div class="referral-loading">
                <i class="fas fa-spinner fa-spin"></i> Loading your referral status...
            </div>
        </div>
            ...
            <div class="referral-share-buttons">
                <button class="referral-share-btn referral-share-whatsapp" onclick="shareReferralVia('whatsapp')">
                    ...
                    <div>
                        ...
                        <span>Send to contacts or groups</span>
                    </div>
                </button>
                <button class="referral-share-btn referral-share-gmail" onclick="shareReferralVia('gmail')">
                    <i class="fas fa-envelope"></i>
                    <div>
                        <strong>Share via Gmail</strong>
        ...
        <div class="referral-preview-section">
            <h2 class="referral-section-title"><i class="fas fa-eye"></i> Preview Share Content</h2>
            <div class="referral-preview-tabs">
                <button class="referral-preview-tab active" onclick="previewReferralContent('whatsapp', this)">
                    <i class="fab fa-whatsapp"></i> WhatsApp Preview
                </button>
                <button class="referral-preview-tab" onclick="previewReferralContent('gmail', this)">
                    <i class="fas fa-envelope"></i> Gmail Preview
                </button>
            </div>
            <div class="referral-preview-box" id="referral-preview-content">
                <div class="referral-loading">
...
async function loadReferralStatus() {
    const container = document.getElementById('referral-stats-container');
    if (!container) return;

    try {
        const response = await fetch(`${BACKEND_URL}/referrals/status`, {
            headers: { 'Authorization': `Bearer ${appState.jwtToken}` }
        });
        ...
        <div class="referral-stats-grid">
            <div class="referral-stat-card referral-stat-primary">
                <div class="referral-stat-icon"><i class="fas fa-gift"></i></div>
                <div class="referral-stat-value">${data.rewardsAvailable}</div>
        ...
        <div class="referral-progress-section">
            <h3 class="referral-progress-title">
                <i class="fas fa-tasks"></i> Progress to Next ${data.rewardType}
            </h3>
        ...
        <div class="referral-code-section">
            ...
            <div class="referral-code-box">
                <span class="referral-code-value">${data.referralCode}</span>
                <button class="referral-copy-btn" onclick="copyReferralCode('${data.referralCode}')">
            ...
            <div class="referral-history-list">
                ${data.rewardHistory.map(r => `
                    <div class="referral-history-item">
                        <div class="referral-history-icon ${r.status === 'available' ? 'available' : 'used'}">
                            <i class="fas ${r.status === 'available' ? 'fa-gift' : 'fa-check'}"></i>
                        </div>
...
async function previewReferralContent(platform, tabEl) {
    ...
    const previewBox = document.getElementById('referral-preview-content');
    if (!previewBox) return;

    previewBox.innerHTML = '<div class="referral-loading"><i class="fas fa-spinner fa-spin"></i> Generating preview...</div>';

    try {
        const response = await fetch(`${BACKEND_URL}/referrals/share-content?platform=${platform}`, {
            headers: { 'Authorization': `Bearer ${appState.jwtToken}` }
        });
        const result = await response.json();
        if (!result.success) throw new Error(result.message);
        ...
        previewBox.innerHTML = `<div class="referral-preview-error"><i class="fas fa-exclamation-circle"></i> Could not load preview. Please try again.</div>`;
    }
}
...
async function shareReferralVia(platform) {
    try {
        const response = await fetch(`${BACKEND_URL}/referrals/share-content?platform=${platform}`, {
            headers: { 'Authorization': `Bearer ${appState.jwtToken}` }
        });
...
async function useReferralReward() {
    const userType = appState.currentUser?.type || 'contractor';
