...
function renderAppSection(sectionId) {
    ...
    if (sectionId === 'jobs') {
        ...
        // THIS NOW CALLS THE CORRECT, ENHANCED ESTIMATOR
...
// --- ENHANCED STEEL TONNAGE ESTIMATOR (REPLACES OLD VERSION) ---
                ...
                <form id="tonnage-estimator-form" class="modern-form enhanced-form">
                    ...
                    <div class="form-step active" data-step="1">
                        <div class="form-section">
                            ...
                            <div id="uploaded-files-container" class="uploaded-files-display"></div>
                                                        <!-- File Viewer -->
                            <div id="file-viewer-container" class="file-viewer-container" style="display: none;">
                                <div class="viewer-header">
                                    <h4 id="viewer-title">File Viewer</h4>
                                    <button class="btn btn-sm btn-secondary" onclick="closeFileViewer()">
                                        <i class="fas fa-times"></i> Close
                                    </button>
                                </div>
                                <div id="file-viewer-content" class="viewer-content"></div>
                            </div>
                        </div>
                    ...
                    <div class="form-step" data-step="2">
                        <div class="form-section">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    <label class="form-label" for="structureType">
                                ...
                                <div class="form-group">
                                    <label class="form-label" for="projectComplexity">
                                        <i class="fas fa-layer-group"></i> Project Complexity
                                    </label>
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    <label class="form-label" for="steelGrade">
                                        <i class="fas fa-industry"></i> Primary Steel Grade
                                    </label>
                                ...
                                <div class="form-group">
                                    <label class="form-label" for="coatingRequirement">
                                        <i class="fas fa-paint-brush"></i> Coating Requirement
                                    </label>
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    <label class="form-label" for="region">
                                        <i class="fas fa-globe"></i> Region/Country
                                    </label>
                    ...
                    <div class="form-step" data-step="3">
                        <div class="form-section">
                            <h3><i class="fas fa-weight-hanging"></i> Tonnage & Final Review</h3>
                                ...
                                <div class="tonnage-display-card">
                                    <div class="tonnage-icon">
                                        <i class="fas fa-balance-scale"></i>
                                    </div>
                                    <div class="tonnage-content">
                                        <label class="form-label" for="totalTonnageInput">
                                            Total Steel Tonnage (Metric Tons)
                                        </label>
                                        <div class="tonnage-input-group">
                                            <input type="number" id="totalTonnageInput" class="form-input tonnage-input"
                                                    placeholder="Enter tonnage" step="0.01" min="0.1" required>
                                            <div class="tonnage-unit">MT</div>
                                        </div>
                                        <small class="form-help">
                                            <i class="fas fa-info-circle"></i>
                                            Auto-filled from uploaded files or enter manually
                                        </small>
                                    </div>
                                </div>
                                <!-- Tonnage Breakdown -->
                                <div id="tonnage-breakdown" class="tonnage-breakdown" style="display: none;">
                                    <h4>Detected Steel Components</h4>
                                    <div id="tonnage-breakdown-content"></div>
                                </div>
                            </div>
                            <!-- Project Summary -->
                            <div class="project-summary-card">
                                <h4><i class="fas fa-clipboard-list"></i> Project Summary</h4>
                                <div id="project-summary-content" class="summary-grid">
                        ...
                        <div class="step-navigation">
                            ...
                            <button type="button" id="calculate-estimate-btn" class="btn btn-primary btn-calculate">
                                <i class="fas fa-calculator"></i> Generate Professional Estimate
                            </button>
                        </div>
                    </div>
                </form>
            ...
            <div class="tonnage-results-panel">
                <div id="tonnage-result-container" class="results-wrapper">
                    <div class="results-placeholder">
                        ...
                        <p>Complete the form steps to generate your detailed cost estimation report with:</p>
...
function attachFormEventListeners() {
    ...
    // Real-time project summary updates
...
function processUploadedFiles(files, type) {
    files.forEach(file => {
        if (validateFileType(file, type)) {
            ...
            tonnageEstimatorState.currentFiles.push({
                file,
                type,
                id: fileId,
                status: 'processing'
...
function displayUploadedFile(file, type, fileId) {
    ...
    fileElement.innerHTML = `
        ...
        <div class="file-actions">
            ...
            <button class="btn btn-sm btn-outline" onclick="viewFile('${file.name}', '${type}')" style="display: none;" id="view-${fileId}">
...
function processFileForTonnage(file, type, fileId) {
    // Simulate processing and tonnage extraction
    setTimeout(() => {
        const statusElement = document.getElementById(`status-${fileId}`);
        const viewButton = document.getElementById(`view-${fileId}`);
...
function removeFile(fileId) {
    const fileToRemove = tonnageEstimatorState.currentFiles.find(f => f.id === fileId);
    if (!fileToRemove) return;

    // This is a placeholder for a real implementation
    // For now, we just remove from UI and assume tonnage is manually adjusted
...
function generateRealisticTonnage(type) {
    // More realistic tonnage generation based on file type
    const baseRanges = {
        'mto': { min: 50, max: 800 },
        'dwg': { min: 20, max: 400 },
        'model': { min: 100, max: 1200 },
        'spec': { min: 0, max: 0 }
    };
...
function getFileIcon(filename) {
    ...
    const iconMap = {
        ...
        'dwg': '<i class="fas fa-drafting-compass"></i>', 'dxf': '<i class="fas fa-vector-square"></i>',
...
function getFileIconClass(filename) {
    ...
    const classMap = {
        ...
        'pdf': 'pdf', 'dwg': 'cad', 'dxf': 'cad',
        'ifc': 'model', 'step': 'model', 'stp': 'model', 'sat': 'model',
        'doc': 'word', 'docx': 'word', 'txt': 'text'
    };
...
function previousStep(stepNumber) {
    // Just move to the previous step without re-validating
    document.querySelectorAll('.progress-step').forEach(step => {
        const stepNum = parseInt(step.dataset.step);
        step.classList.toggle('active', stepNum <= stepNumber);
        step.classList.toggle('completed', stepNum < stepNumber);
...
function validateCurrentStep(stepNumber) {
    switch(stepNumber) {
        ...
        case 2:
            ...
            return true;
        case 3:
             if (parseFloat(document.getElementById('totalTonnageInput').value) > 0) {
                 return true;
             }
...
function handleCalculateEnhancedEstimate() {
    ...
    tonnageEstimatorState.currentEstimate = {
        ...
        breakdown: {
            ...
            'Engineering & Management (8%)': engineeringServices,
            'Transport & Logistics (5%)': transportLogistics,
            'Quality Control (3%)': qualityControl,
...
function downloadTonnageReport() {
    ...
    let reportContent = `
STEEL STRUCTURE COST ESTIMATION REPORT
==========================================
...
- Generated by SteelConnect Professional Estimator.
    `.trim();
